...
def triangle():
    ...
    tri.draw(win)
    length_1 = math.sqrt(abs((p2.getX() - p1.getX()) + (p2.getY() - p1.getY())))
    length_2 = math.sqrt(abs((p3.getX() - p2.getX()) + (p3.getY() - p2.getY())))
    length_3 = math.sqrt(abs((p3.getX() - p1.getX()) + (p3.getY() - p1.getY())))
    ...
    area_t = math.sqrt(side_s * a * b * c)
    area_message = Text(Point(250, 350), "area: " + str(round(area_t, 2)))
    area_message.draw(win)
    peri = 2 * (length + width)
    perimeter_message = Text(Point(250, 450), "perimeter: " + str(round(peri, 2)))
    perimeter_message.draw(win)
    tri.setFill("pink")
    tri.setOutline("yellow")
    win.getMouse()
    win.close()
